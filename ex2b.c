...
void Process(char** Command,int * How_Many_process,int * countOfLetter,int countOfWords, char sched[]);// this method will take the Shell command and activate it
...
int main()
{
    ...
    *How_Many_process=*countOfLetter=0;
    ...
    for(;;) {//takes command from the user
        ...
        Process(Command,How_Many_process,countOfLetter,countOfWords,sched);
        strcpy(sched, "");
    }
}
int howManyWords(const char str[],int * How_Many_process,int *countOfLetter,int countOfWords) { // the method will calculate how many words we entered through the string
...
void Process(char** Command,int * How_Many_process,int * countOfLetter,int countOfWords,char sched[]){// this method will take the Shell command and activate it
    ...
    if (child1 < 0) { /* error occurred */
        ...
        child2 = fork();/* fork another process */
        if (child2 < 0) { /* error occurred */
            perror("folk filed");
            exit(1);
        }
        else if (child2 == 0) {/* child2 process */
            snprintf( sched, MAX+2,"/proc/%d/sched", getppid()); // turn the child1 PID to String
            char *schedulerCommand[] = {"cat", sched, NULL};//here the command will get the array to use it in the function scheduler
            execvp(schedulerCommand[0], schedulerCommand);//does the function scheduler
            perror("command not found");//if the execvp filed
            freeDynamicAllocation(Command, countOfWords);//free malloc
            free(How_Many_process);
            free(countOfLetter);
            exit(1);
        }
        else {
            wait(NULL);//wait until the child 2 ends
